...
static void import_kernel_nv(char *name, bool for_emulator)
{
    ...
    *value++ = 0;
    ...
    if (!strcmp(name,"frdump_boot")) {
        property_set( "sys.mtk.frdumpboot", "1" );
    }
#endif
...
int main(int argc, char** argv) {
    ...
    if (is_first_stage) {
        if (restorecon("/sbin/multi_init") == -1) {
    ...
    //NEW FEATURE: multi-boot mode
    int mt_boot_mode = 0;
    NOTICE("multi_init get boot mode\n");
    mt_boot_mode = get_boot_mode();
    if ((mt_boot_mode == MT_FACTORY_BOOT) || (mt_boot_mode == MT_ATE_FACTORY_BOOT)) {
#ifdef MTK_TC7_FEATURE
        char tmp[PROP_VALUE_MAX];
        if((property_get("sys.mtk.frdumpboot", tmp) != 0) && (strcmp(tmp, "1") == 0)) {
            NOTICE("FRDUMP Mode Booting.....\n");
            init_parse_config_file("/frdump_init.rc");
        } else {
            NOTICE("Factory Mode Booting.....\n");
