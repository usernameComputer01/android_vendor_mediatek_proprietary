...
public class AudioModeSetting extends Activity implements OnClickListener {
    ...
    private class ValLevelItemSelectListener implements OnItemSelectedListener {
        ...
        public void onItemSelected(AdapterView<?> arg0, View arg1,
            ...
            setMaxVolEdit();
            Log.v("@M_" + TAG, "SLevel: " + mCurrentMode + " " + mTypeIndex + " "
                    + mLevelIndex);

        }
        @Override
        public void onNothingSelected(AdapterView<?> arg0) {
            Log.v("@M_" + TAG, "noting selected.");
        }
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Log.v("@M_" + TAG, "mCurrentMode: " + mCurrentMode + " mSupportEnhance: "
                + mSupportEnhance);
        if (mSupportEnhance) {
            ...
            if (dataSize != sStructSize) {
                Log.d("@M_" + TAG, "assert! Check the structure size!");
        ...
        if (ret != 0) {
            ...
            Log.i("@M_" + TAG, "AudioModeSetting GetAudioData return value is : "
                    + ret);
        }
        ...
        if (mSupportEnhance) {
            ...
            Log.d("@M_" + TAG, "mRealUsageVols: " + debugStr + "]");
        }
        ...
        mVolTypeSpinner.setOnItemSelectedListener(new OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> arg0, View arg1,
                ...
                if (mSupportEnhance) {
                    mTypeIndex = getVolumeTypeIndex(itemName);
                } else {
                    if (mCurrentMode == 0) {
                        if (arg2 == 0 || arg2 == 1) {
                            mTypeIndex = arg2 + 1;
                ...
                Log.d("@M_" + TAG, "mTypeIndex is:" + mTypeIndex);
                // Mode:Sph/Sid/Mic
                if (mTypeIndex == sModeSphIndex || mTypeIndex == sModeSph2Index
                        || mTypeIndex == sModeSidIndex
                        || mTypeIndex == sModeMicIndex) {
                    mEditMaxVol.setEnabled(true); // false
                    mBtnSetMaxVol.setEnabled(true); // false
                    if (mSupportEnhance) {
                        mEditMaxVolSpeaker.setEnabled(true); // false
                        mBtnSetMaxVolSpeaker.setEnabled(true); // false
                    }
                ...
                mValueEdit.setText(String.valueOf(getValue(mData, mCurrentMode,
                        mTypeIndex, mLevelIndex)));
                setMaxVolEdit();
                Log.v("@M_" + TAG, "SMode: " + mCurrentMode + " " + mTypeIndex + " "
                        + mLevelIndex);
                // update level spinner for 6589
                if (mSupportEnhance) {
                    Log.d("@M_" + TAG, "itemName: " + itemName);
                ...
                public void onItemSelected(AdapterView<?> arg0, View arg1,
                    ...
                    if (mCurrentMode == 0) { // normal mode
                        ret = AudioSystem.setAudioCommand(AUDIO_COMMAND_PARAM0,
                                arg2);
                        Log.v("@M_" + TAG, "set normal fir Z" + arg2);
                        // headset mode
                    } else if (mCurrentMode == 1) {
                        ret = AudioSystem.setAudioCommand(AUDIO_COMMAND_PARAM1,
                                arg2);
                        Log.v("@M_" + TAG, "set headset fir Z" + arg2);
                        // loudspeaker mode
                    } else if (mCurrentMode == 2) {
                        ret = AudioSystem.setAudioCommand(AUDIO_COMMAND_PARAM2,
                                arg2);
                        Log.v("@M_" + TAG, "set loudspeaker fir Z" + arg2);
                    }
                    if (-1 == ret) {
    ...
    private int getVolumeTypeIndex(String volTypeStr) {
    ...
    private void setValue(byte[] dataPara, int mode, int type, int level,
        ...
        if (dataPara == null || mode >= sMaxVolMode || type >= sMaxVolType
                || level >= sMaxVolLevel) {
            Log.d("@M_" + TAG, "assert! Check the setting value.");
            return;
        }
        Log.d("@M_" + TAG, "setValue() mode:" + mode + ", type:" + type + ", level:" + level);
    ...
    private void setAudioData() {
        ...
        if (0 == result) {
            ...
            Log.i("@M_" + TAG, "AudioModeSetting SetAudioData return value is : "
                    + result);
        }
    }
